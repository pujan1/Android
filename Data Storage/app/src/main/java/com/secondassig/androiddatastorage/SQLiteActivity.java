...
public class SQLiteActivity extends Activity {
	...
	{
		EditText editText=(EditText)findViewById(R.id.msg);  //initializing the textboxes
		EditText editText1=(EditText) findViewById(R.id.editText);
		EditText editText2=(EditText) findViewById(R.id.edit_desc);

		String Author=editText1.getText().toString();  //assigning the text boxes to a string
		String Description = editText2.getText().toString();
		String message=editText.getText().toString();

		DataController dataController=new DataController(getBaseContext());
		dataController.open();
		long retValue= dataController.insert(message, Author, Description); // inserting name, author and desc values to controller
		dataController.close();
		if(retValue!=-1)
		{
			Context context = getApplicationContext();
    		CharSequence text=getString(R.string.save_success_msg); // toast to show the successful save
